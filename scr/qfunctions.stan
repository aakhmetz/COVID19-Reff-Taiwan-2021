// from https://github.com/bgoodri/NYU2020/blob/master/Week2/quantile_functions.stan
  ...
  /*
     ...
     @return real number greater than lower_bound
   */
  real JQPDS2_icdf(real p, real lower_bound, real alpha, vector quantiles) {
    if (p < 0 || p > 1)         reject("p must be between 0 and 1");
    if (alpha < 0 || alpha > 1) reject("alpha must be between 0 and 1");
    if (rows(quantiles) != 3)   reject("quantiles must have three elements");
    if (in_order({lower_bound, quantiles[1], quantiles[2], 
  ...
  /*
     ...
     @return real number greater than lower_bound
   */
  real JQPDS2_rng(real lower_bound, real alpha, vector quantiles) {
  ...
  /*
     ...
     @return real number greater than lower_bound
   */
  real JQPDB_icdf(real p, row_vector bounds, real alpha, vector quantiles) {
